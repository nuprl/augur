...
function parseUsersWin(lines) {
  ...
  for (var i = 1; i < lines.length; i++) {
    if (lines[i].trim()) {
      var user = lines[i].substring(headerDelimiter[0]+1, headerDelimiter[1]).trim() || '';
      var tty = lines[i].substring(headerDelimiter[1]+1, headerDelimiter[2] - 2).trim() || '';
      var dateTime = util.parseDateTime(lines[i].substring(headerDelimiter[5]+1, 2000).trim()) || '';
      result.push({
        user: user,
        tty: tty,
        date: dateTime.date,
